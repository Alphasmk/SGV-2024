...
namespace CG
{
	std::string convertToChar(const wchar_t* wideString) {
		// Определяем длину результата
		int size_needed = WideCharToMultiByte(CP_UTF8, 0, wideString, -1, nullptr, 0, nullptr, nullptr);

		// Выделяем место для результата
		std::string result(size_needed, 0);

		// Конвертируем
		WideCharToMultiByte(CP_UTF8, 0, wideString, -1, &result[0], size_needed, nullptr, nullptr);

		return result;
	}

	Generator::Generator(LT::LexTable _lt, IT::IdTable _it, OT::OpTable _ot, Parm::PARM parm)
	{
		lt = _lt;
		it = _it;
		ot = _ot;
	...
	void Generator::Head()
	{
		...
		out << "\tincludelib E:\\unic\\courseKPO\\Project1\\Debug\\SGVLIB.lib\n";
	...
	void Generator::Code()
	{
		...
		for (int i = 0; i < lt.size; i++)
		{
			...
			else if (lt.table[i].lexema == '{')
			{
				...
				if (flag_else && lt.table[i - 1].lexema == LEX_ELSE)
				{
					out << "else" << ifi.top() << ":\n";
				}
			}
			...
			else if (lt.table[i].lexema == 'i' && it.table[lt.table[i].idxTI - 1].idtype == IT::F && strcmp(it.table[lt.table[i].idxTI - 1].id, "_exp") != 0 && strcmp(it.table[lt.table[i].idxTI - 1].id, "_pow") != 0 && strcmp(it.table[lt.table[i].idxTI - 1].id, "_pause") != 0)
			{
				if (strcmp(it.table[lt.table[i].idxTI - 1].id, "_printInt") == 0)
				{
					out << "\npush " << it.table[lt.table[i + 2].idxTI - 1].id << "\ncall " << it.table[lt.table[i].idxTI - 1].id << "\n";
				}
				else if (strcmp(it.table[lt.table[i].idxTI - 1].id, "_printStr") == 0)
				{
				...
				else if (lt.table[i - 1].lexema == ';')
				{
					int funcIdBuf = i;
					i++;
					while (lt.table[i].lexema != ')')
					{
						if (lt.table[i].lexema == 'i' || lt.table[i].lexema == 'l' && it.table[lt.table[i].idxTI - 1].idtype != IT::F)
						{
							if (it.table[lt.table[i].idxTI - 1].iddatatype == IT::STR && (it.table[lt.table[i].idxTI - 1].id[0] == 'L'))
							{
								out << "\npush offset " << it.table[lt.table[i].idxTI - 1].id << "\n";
							}
							else if (it.table[lt.table[i].idxTI - 1].iddatatype == IT::STR)
							{
								out << "\npush " << it.table[lt.table[i].idxTI - 1].id << "\n";
							}
							else if ((it.table[lt.table[i].idxTI - 1].iddatatype == IT::CHAR))
							{
								out << "\npush " << it.table[lt.table[i].idxTI - 1].id << "\n";
					...
					out << "\n\tcall " << it.table[lt.table[funcIdBuf].idxTI - 1].id << "\n";
					i++;
				}
			}
