...
namespace SemAnalize
{
	...
	void CheckOperations(LT::LexTable lt, IT::IdTable it)
	{
		...
		for (i; i < lt.size; i++)
		{
							...
							while (true)
							{
								if (lt.table[k].lexema == 'i' || lt.table[k].lexema == 'l')
								{
									for (int j = 0; j < params.size(); j++)
									{
										...
										{
											funId = j;
											if (paramCounter + 1 <= params[j].second.size())
											{
			...
			else if (lt.table[i].lexema == 'c')
			{
				...
				{
					ERROR_THROW_IN(713, sn, il);
				}
			}
			else if (it.table[lt.table[i].idxTI - 1].idtype == IT::F && lt.table[i].lexema == 'i')
			{
				string buf = it.table[lt.table[i].idxTI - 1].id;
				int funCheck = i + 2;
				int k = i + 2;
				int paramCounter = 0, funId = 0;
				while (lt.table[funCheck].lexema != ')')
				{
					if (lt.table[funCheck].lexema != 'i' && lt.table[funCheck].lexema != 'l' && lt.table[funCheck].lexema != ',')
					{
						ERROR_THROW_IN(711, sn, il);
					}
					if (lt.table[funCheck].lexema == 'i' || lt.table[funCheck].lexema == 'l')
					{
						for (int j = 0; j < params.size(); j++)
						{
							if (params[j].first == buf)
				...
				for (int j = 0; j < params.size(); j++)
				{
					...
					{
						funId = j;
						if (paramCounter != params[j].second.size())
						{
							ERROR_THROW_IN(711, sn, il);
						}
					}
	...
	void CheckIf(LT::LexTable lt, IT::IdTable it)
	{
		for (int i = 0; i < lt.size; i++)
		{
			if (lt.table[i].lexema == 's')
			{
				i += 2;
				while (lt.table[i].lexema != ')')
				{
					if (it.table[lt.table[i].idxTI - 1].iddatatype == IT::F)
					{
						ERROR_THROW_IN(714, lt.table[i].sn, i);
					}
					i++;
				}
			}
